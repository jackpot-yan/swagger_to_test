...
func (w *connWriter) Close() error {
    // 写入结束标记(4字节的0)
    err := binary.Write(w.conn.conn, binary.BigEndian, uint32(0))
    w.conn.sendMu.Unlock()
    return err
}
...
func (conn *Conn) Send(key string) (writer io.WriteCloser, err error) {
    ...
    // 发送key长度
    if err := binary.Write(conn.conn, binary.BigEndian, uint32(len(key))); err != nil {
        conn.sendMu.Unlock()
        return nil, err
    }

    // 发送key内容
    if _, err := conn.conn.Write([]byte(key)); err != nil {
        conn.sendMu.Unlock()
        return nil, err
    }

    return &connWriter{conn: conn}, nil
}
...
func (r *connReader) Read(p []byte) (n int, err error) {
    ...
    // 检查是否包含结束标记
    if n >= 4 {
        endMarker := binary.BigEndian.Uint32(p[n-4:])
        if endMarker == 0 {
            r.conn.recvMu.Unlock()
            return n - 4, io.EOF
        }
    }
    return n, nil
...
func (conn *Conn) Receive() (key string, reader io.Reader, err error) {
    ...
    // 读取key长度
    var keyLen uint32
    if err := binary.Read(conn.conn, binary.BigEndian, &keyLen); err != nil {
        conn.recvMu.Unlock()
        return "", nil, err
    }
    ...
    if _, err := io.ReadFull(conn.conn, keyBuf); err != nil {
        conn.recvMu.Unlock()
        return "", nil, err
    }

    return string(keyBuf), &connReader{conn: conn}, nil
}
